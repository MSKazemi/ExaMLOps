...
class FDataDataset(SeanergysParquetDataset):
    ...
    def load_data(self) -> None:
        ...
        try:
            if self.use_zenodo_url and self._file_list:
                self.logger.info(
                    f"Loading {len(self._file_list)} F-DATA file(s): "
                    f"{self._file_list[0]} to {self._file_list[-1]}"
                )

                dfs = []
                for file_name in self._file_list:
                    # Changed by Mohsen: original code saved the merged post-processed file
                    # (missing the 'adt' column) causing parquet filter errors on re-load.
                    # Each monthly file is now cached as {file_name}_full.parquet with all
                    # columns so any split/filter combination works without re-downloading.
                    cached_file = (
