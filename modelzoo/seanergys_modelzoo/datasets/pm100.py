...
class PM100Dataset(SeanergysParquetDataset):
    ...
    def validate_model(self) -> 'PM100Dataset':
        """Resolve data_path from Zenodo URL if needed, enrich metadata, and trigger data loading."""
        # Changed by Mohsen: cache now stores the FULL raw file (all columns, no filters) as
        # job_table_full.parquet. The original code saved a post-processed file that was missing
        # filter columns (e.g. submit_time), causing parquet filter errors on re-load. The full
        # cache lets any split/filter combination work without re-downloading.
        if self.data_path is None and self.use_zenodo_url:
            ...
            raise ValueError(
                "data_path must be provided or use_zenodo_url must be True."
            )
