...
from seanergys_modelzoo.datasets._backends import DatasetBackend
...
class SeanergysDataset(BaseModel, Dataset, ABC):
    ...
    backend: Optional[DatasetBackend] = Field(
        default=None,
        description=(
            "Optional pluggable storage backend (zenodo / minio / dataplane). "
            "When set, subclasses consult it to resolve dataset paths instead "
            "of using their hard-coded ZENODO_URL. None preserves legacy behaviour."
        ),
    )
