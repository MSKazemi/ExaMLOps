"""
DataPlane → ExaMLOps Ray Serve bridge.

Connects a Cap'n Proto / TCP dataplane server to ExaMLOps's Ray Serve inference
API. Supports three modes via DATAPLANE_MODE:
...
Env vars (defaults shown):
    ...
    DATAPLANE_PORT=<PORT>
    DATAPLANE_MODE=both
    DATAPLANE_JOB_TOPIC_UUID        required for pubsub / both
    DATAPLANE_RESULT_TOPIC_UUID     optional; enables result publishing
    DATAPLANE_INFERENCE_UUID        required for reqres / both
    DATAPLANE_RETRAIN_UUID          required for reqres / both
    DATAPLANE_VECTOR_UUID           required for vector req/res; omit to disable
    ...
    DATAPLANE_PUBLISH_RESULTS=true
    DRIFT_WINDOW=50
    DRIFT_THRESHOLD=0.5
    DRIFT_COOLDOWN=300
...
# Allow importing sibling packages when run as `python clients/dataplane_bridge.py`
...
from dataplane_msgs import (  # noqa: E402
    HpcInferenceResV1,
    HpcJobV1,
    RetrainReqV1,
    RetrainResV1,
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [dataplane-bridge] %(levelname)s: %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger("dataplane_bridge")
...
DATAPLANE_MODE = os.getenv("DATAPLANE_MODE", "both").lower()
...
DEFAULT_MODEL = os.getenv("DATAPLANE_DEFAULT_MODEL", "JPCP")
DEFAULT_ALIAS = os.getenv("DATAPLANE_DEFAULT_ALIAS", "Production")
PUBLISH_RESULTS = os.getenv("DATAPLANE_PUBLISH_RESULTS", "true").lower() == "true"
...
JOB_TOPIC_UUID = _parse_uuid("DATAPLANE_JOB_TOPIC_UUID")
RESULT_TOPIC_UUID = _parse_uuid("DATAPLANE_RESULT_TOPIC_UUID")
INFERENCE_UUID = _parse_uuid("DATAPLANE_INFERENCE_UUID")
RETRAIN_UUID = _parse_uuid("DATAPLANE_RETRAIN_UUID")
VECTOR_UUID = _parse_uuid("DATAPLANE_VECTOR_UUID")


def _load_model_uuids() -> dict[str, uuid.UUID]:
    """Read dataplane_uuid from every model YAML in MODELS_YAML_DIR.

    Returns {UPPERCASE_MODEL_NAME: uuid.UUID}. Models without dataplane_uuid
    ...
    for yaml_file in sorted(models_path.glob("*.yaml")):
        ...
        try:
            ...
            continue
        raw_uuid = raw.get("dataplane_uuid", "")
        model_name = raw.get("name", "")
        if raw_uuid and model_name:
            try:
                result[model_name.upper()] = uuid.UUID(raw_uuid)
            except ValueError:
                log.warning("Invalid dataplane_uuid in %s — skipping", yaml_file.name)
    return result
...
async def _run_pubsub(conn: Connection) -> None:
    if JOB_TOPIC_UUID is None:
        log.warning("DATAPLANE_JOB_TOPIC_UUID not set — pubsub mode disabled")
        return
    ...
    if PUBLISH_RESULTS and RESULT_TOPIC_UUID is not None:
        result_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
    ...
    async for job in conn.subscribe(JOB_TOPIC_UUID, HpcJobV1):
        ...
        if result_conn is not None and RESULT_TOPIC_UUID is not None:
            ...
            try:
                ...
                log.error("Failed to publish result to dataplane: %s", exc)
...
async def _run_reqres(inf_conn: Connection, retrain_conn: Connection, vector_conn: Connection) -> None:
    ...
    if MODEL_UUIDS:
        ...
        log.warning("No inference handlers registered — set DATAPLANE_INFERENCE_UUID or add dataplane_uuid to model YAMLs")
...
async def main() -> None:
    _BRIDGE_UP.set(1.0)
    _bridge_stats["mode"] = DATAPLANE_MODE
    log.info(
        "DataPlane bridge starting | host=%s port=%d mode=%s",
        DATAPLANE_HOST, DATAPLANE_PORT, DATAPLANE_MODE,
    )

    async def _run_bridge() -> None:
        if DATAPLANE_MODE == "pubsub":
            conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            await conn.connect()
            await _run_pubsub(conn)

        elif DATAPLANE_MODE == "reqres":
            inf_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            retrain_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            vector_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
        ...
        elif DATAPLANE_MODE == "both":
            pubsub_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            inf_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            retrain_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            vector_conn = Connection(DATAPLANE_HOST, DATAPLANE_PORT)
            ...
            log.error("Unknown DATAPLANE_MODE=%r — use pubsub | reqres | both", DATAPLANE_MODE)
    ...
    async def _run_bridge_safe() -> None:
        ...
        while True:
            ...
            try:
                if attempt > 1:
                    log.info("Reconnecting to DataPlane (attempt %d) …", attempt)
                await _run_bridge()
                log.info("Bridge exited cleanly")
                break
            except EOFError as exc:
                log.warning("DataPlane connection closed: %s — reconnecting in %.0fs", exc, backoff)
            except Exception:
