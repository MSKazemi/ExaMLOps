...
_EX = (
    ...
    "  [dim]# ...and add 8 GPUs to cluster remote, carbon-aware placement[/dim]\n"
    "  exa fleet simulate --jobs 20 --gpus 2 --add-gpus remote=8 --optimize carbon-aware"
)
...
def simulate(
    ...
    add_gpus: list[str] = typer.Option(
        None, "--add-gpus", help="Add idle GPUs, e.g. --add-gpus remote=8 (repeatable)"
    ),
    carbon: list[str] = typer.Option(
        None, "--carbon", help="Override grid carbon, e.g. --carbon remote=600 (repeatable)"
    ),
