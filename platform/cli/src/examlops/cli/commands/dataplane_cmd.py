...
def _insert_uuid(text: str, uid: str) -> str:
    """Insert dataplane_uuid after the 'enabled:' line (preserves all formatting)."""
    m = re.search(r"^enabled:.*$", text, re.MULTILINE)
    if m:
        pos = m.end()
        return text[:pos] + f"\ndataplane_uuid: {uid}" + text[pos:]
    return text.rstrip() + f"\ndataplane_uuid: {uid}\n"


def _replace_uuid(text: str, uid: str) -> str:
    """Replace existing dataplane_uuid value in YAML text."""
    return re.sub(
        r"^dataplane_uuid:.*$",
        f"dataplane_uuid: {uid}",
        text,
        count=1,
        flags=re.MULTILINE,
    )
...
def list_uuids():
    ...
    for name, path, text in _iter_yamls():
        ...
        rows.append(
            [
                name,
                raw.get("dataplane_uuid") or "(not assigned)",
            ]
        )
    _output.print_table("DataPlane UUIDs", ["Model", "UUID"], rows)
...
def init_uuids():
    ...
    for name, path, text in _iter_yamls():
        raw = yaml.safe_load(text) or {}
        if raw.get("dataplane_uuid") is not None:
            continue
...
def status():
    ...
    cfg = load_config()
    base_url = getattr(cfg, "dataplane_bridge_url", "http://localhost:18003")
    try:
