...
def evidence_refusals(row: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    ...
    agent, vid, manifest = row["agent"], row["version_id"], row["manifest"]
    key = model_key(agent)
    ...
    for suite in suites:
        got = scored(suite)
        if not got:
            reasons.append(f"no evaluation evidence: suite {suite!r} has no results for {vid}")
            continue
        judge = judge_for_results(key, suite, vid)
        if judge:
    ...
    if missing:
        reasons.append(f"no score for gate metric(s) {missing} on {vid}")
    if reasons:
        return reasons, evidence
    result = run_eval_gate(key, vid, persist=True)
