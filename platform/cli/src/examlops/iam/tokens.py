...
def _principal(provider: ProviderConfig, claims: dict[str, Any], token_type: str) -> Principal:
...
def verify_access_token(
    ...
    if looks_like_jwt(token):
        ...
        return _principal(provider, claims, "jwt")
    provider, claims = introspect(token, config, provider_hint=provider_hint)
    return _principal(provider, claims, "opaque")
