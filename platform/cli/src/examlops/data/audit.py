...
def _lock_chain_head(conn: Any) -> None:
    ...
    if type(conn).__name__ == "PgConnection":
        ...
        # Nothing is open yet, so a lost lock race is simply retried (busy_timeout waits once;
        # a saturated host can outlast it).
        write_retry(lambda: conn.execute("BEGIN IMMEDIATE"))
