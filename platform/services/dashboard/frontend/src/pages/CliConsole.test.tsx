...
const CREATED = {
  ...
  display: 'exa project create -- console-p1',
  tier: 'admin',
  format: 'table',
  actor: 'dashboard:admin@x',
  status: 'succeeded',
  exit_code: 0,
...
const apiFetch = vi.fn((path: string, opts?: { method?: string; body?: string }) => {
  ...
  if (path === '/api/v1/cli/runs') return Promise.resolve({ runs: historyRuns })
  if (path.startsWith('/api/v1/cli/runs/r2')) return Promise.resolve(CREATED)
...
describe('CLI Console', () => {
  ...
  it('"Run again" loads a stored run back into its form — for review, never auto-run', async () => {
    caps = ['view', 'cli.run', 'cli.write']
    setAuth({ token: 't', role: 'admin', expiresAt: new Date(Date.now() + 3600_000).toISOString() })
    historyRuns = [CREATED]
    renderAt('/platform/cli')
    fireEvent.click(await screen.findByRole('tab', { name: 'history' }))
    fireEvent.click(await screen.findByText('exa project create -- console-p1'))
    fireEvent.click(await screen.findByRole('button', { name: /Run again/ }))

    expect(await screen.findByLabelText(/NAME/)).toHaveValue('console-p1')
    expect(screen.getByText(/Loaded from the run of/)).toBeInTheDocument()
    expect(screen.getByDisplayValue('Text (as in a terminal)')).toBeInTheDocument()
