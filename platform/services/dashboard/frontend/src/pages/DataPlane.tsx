...
const CONFIG_LABELS: Record<string, string> = {
  ...
  dataplane_port:             'Port',
  dataplane_mode:             'Mode',
  dataplane_job_topic_uuid:   'Job Topic UUID',
  dataplane_result_topic_uuid:'Result Topic UUID',
  dataplane_inference_uuid:   'Inference Handler UUID',
  dataplane_retrain_uuid:     'Retrain Handler UUID',
  dataplane_default_model:    'Default Model',
  dataplane_default_alias:    'Default Alias',
  dataplane_bridge_status_url:'Bridge Status URL',
}
...
function BridgeFlowDiagram({ online }: { online: boolean }) {
  ...
  const nodes = [
    { x: 10,  y: 60, label: 'DataPlane',  sub: 'Cap\'n\'Proto :<PORT>' },
...
export default function DataPlane() {
  ...
  const { data: status, isFetching: statusFetching } = useDataplaneStatus()
  const { data: config, isFetching: configFetching } = useDataplaneConfig()
  const { data: modelUuids, isFetching: uuidsFetching } = useDataplaneModelUuids()
  const { data: grafanaPanels, isFetching: panelsFetching } = useDataplaneGrafanaPanels()
  ...
  const refresh = () => qc.invalidateQueries({ queryKey: ['dataplane'] })
  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      >
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div className="space-y-3">
            ...
            <h1 className="text-3xl font-bold tracking-tight gradient-text">DataPlane Bridge</h1>
                  ...
                  <td style={{ padding: '10px 16px', fontFamily: 'monospace', fontSize: '0.82rem', color: uid ? 'var(--foreground)' : 'var(--faint-text)' }}>
                    {uid ?? (
                      <span style={{ color: 'var(--warning-text)', fontFamily: 'inherit', fontSize: '0.8rem' }}>
                        not assigned — run <code style={{ fontSize: '0.78rem' }}>exa dataplane init-uuids</code>
                      </span>
                    )}
                  </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {(
              ...
              >
                ...
                <iframe
                  src={`${grafanaPanels.grafana_url}/d-solo/${grafanaPanels.dashboard_uid}/dataplane-bridge?panelId=${id}&orgId=1&theme=dark&kiosk`}
                  width="100%"
                  height="180"
      ...
      <div className="space-y-2">
        ...
        {configSet ? (
          ...
          >
            ...
            <div>
              <p className="text-sm font-medium" style={{ color: 'var(--foreground)' }}>No DataPlane configuration</p>
      ...
      <div className="space-y-2">
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--border-sm)' }}>
          <LogPanel containerName="dataplane-bridge" token={token} />
        </div>
      </div>
    </div>
  )
