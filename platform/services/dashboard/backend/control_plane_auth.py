...
async def control_plane_token() -> str | None:
    ...
    A token file (``CONTROL_PLANE_TOKEN_FILE``: the dashboard's workload identity, which
    spiffe-helper keeps fresh; ADR 0125) wins, and is read on every call because it expires
    within minutes.
    """
    import os  # noqa: PLC0415

    from examlops.service_auth import token_from_file  # noqa: PLC0415

    path = os.getenv("CONTROL_PLANE_TOKEN_FILE", "").strip()
    if path and (from_file := token_from_file(path)):
        return from_file
    global _token_cache
