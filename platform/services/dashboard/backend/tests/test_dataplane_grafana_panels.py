"""Tests for GET /api/dataplane/grafana-panels endpoint."""
...
async def test_grafana_panels_returns_correct_shape(client, monkeypatch):
    ...
    response = await client.get("/api/dataplane/grafana-panels", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["grafana_url"] == "http://grafana-test:3000"
    assert data["dashboard_uid"] == "examlops-dataplane"
...
async def test_grafana_panels_requires_auth(client):
    response = await client.get("/api/dataplane/grafana-panels")
