...
async def test_bundled_image_is_fetched_server_side_with_the_credential():
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.url.path == "/v1/models/JPCP/images/arch.png"
...
async def test_the_models_router_reads_the_control_plane_with_the_dashboards_credential(
    monkeypatch,
):
    """The regression itself: the router's client, against a control plane that refuses reads
    without a bearer, as the released one does."""
    ...
    real = models.ControlPlaneClient
    monkeypatch.setattr(
        models,
        "ControlPlaneClient",
    ...
    monkeypatch.setattr(settings, "control_plane_token", "env-token")
    control_plane_auth.reset_token_cache()
    try:
        assert await models._control_plane().list_model_names() == ["JPCP"]
    finally:
        control_plane_auth.reset_token_cache()
...
def image_cp(monkeypatch):
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/v1/models/JPCP/images/arch.png":
            return httpx.Response(200, content=b"\x89PNG", headers={"content-type": "image/png"})
        if request.url.path == "/v1/models/JPCP/images/page.html":
