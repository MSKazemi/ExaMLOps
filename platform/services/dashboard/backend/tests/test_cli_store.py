...
def _submit(runner, tmp_path, command="x", actor="dashboard:admin@a", fmt="json"):
...
async def test_another_replica_sees_a_running_commands_output_so_far(db, tmp_path):
    talker = _script(tmp_path, "talk.sh", "echo 'step 1 of 3'\nexec sleep 30")
    owner = cli_runner.CliRunner(store=cli_store.CliRunStore(), python=talker, timeout=60)
    other = cli_runner.CliRunner(store=cli_store.CliRunStore(), python=talker, timeout=60)
    run = _submit(owner, tmp_path, fmt="table")
    try:
        deadline = time.monotonic() + 10
        seen = None
        while time.monotonic() < deadline:
            seen = other.get(run.id)
            if seen is not None and "step 1 of 3" in seen.stdout:
                break
            await asyncio.sleep(0.2)
        assert seen is not None and "step 1 of 3" in seen.stdout, "no live output across replicas"
        assert seen.status == "running"
    finally:
        await owner.cancel(run.id)
        await owner.wait(run.id, timeout=20)
    assert other.get(run.id).status == "cancelled"


async def test_a_late_live_write_never_overwrites_the_final_result(db, tmp_path):
    # The owner's last partial-output write can land after the final one (it runs in a thread
    # the event loop cannot cancel); it must not turn a finished run back into a partial one.
    echo = _script(tmp_path, "echo.sh", "echo 'final output'")
    store = cli_store.CliRunStore()
    runner = cli_runner.CliRunner(store=store, python=echo)
    run = _submit(runner, tmp_path, fmt="table")
    await runner.wait(run.id)
    store.update_live(run.id, stdout="partial", stderr="", truncated=False)
    rec = store.get(run.id)
    assert rec["status"] == "succeeded"
    assert rec["stdout"].strip() == "final output"
