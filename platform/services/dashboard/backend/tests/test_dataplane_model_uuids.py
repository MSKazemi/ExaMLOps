"""Tests for GET /api/dataplane/model-uuids endpoint."""
...
async def test_get_model_uuids_returns_all_models(client, model_yamls, monkeypatch):
    ...
    response = await client.get("/api/dataplane/model-uuids", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["JPCP"] == "<UUID>"
    assert data["MACK"] is None


async def test_get_model_uuids_requires_auth(client):
    response = await client.get("/api/dataplane/model-uuids")
