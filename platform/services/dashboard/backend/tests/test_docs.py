...
async def test_docs_tree_returns_sections(client):
    ...
    response = await client.get("/api/documents/tree", headers=_hdr(token))
...
async def test_docs_tree_requires_auth(client):
    response = await client.get("/api/documents/tree")
...
async def test_docs_content_returns_markdown(client):
    token = await _login(client, VIEWER_PW)
    response = await client.get("/api/documents/content?path=README.md", headers=_hdr(token))
    assert response.status_code == 200
...
async def test_docs_content_requires_auth(client):
    response = await client.get("/api/documents/content?path=README.md")
    assert response.status_code == 401
...
async def test_docs_content_rejects_path_traversal(client):
    token = await _login(client, VIEWER_PW)
    response = await client.get("/api/documents/content?path=../../etc/passwd", headers=_hdr(token))
