"""DataPlane bridge config and status endpoints."""
...
_DEFAULT_BRIDGE_STATUS_URL = os.getenv("DATAPLANE_BRIDGE_STATUS_URL", "http://localhost:8003")

log = logging.getLogger("dashboard.dataplane")

router = APIRouter(prefix="/dataplane", tags=["dataplane"])

_DATAPLANE_KEYS: frozenset[str] = frozenset(k for k in URL_KEYS if k.startswith("dataplane_"))


async def _get_dataplane_config(db: AsyncSession) -> dict[str, str]:
    stmt = select(DashboardConfig).where(DashboardConfig.key.in_(_DATAPLANE_KEYS))
    rows = (await db.execute(stmt)).scalars().all()
    return {r.key: r.value for r in rows if r.value}


@router.get("/config")
async def get_dataplane_config(
    db: AsyncSession = Depends(get_db),
    _role: str = Depends(require_role("viewer")),
) -> dict:
    """Return all DataPlane config keys (plaintext only, no secrets)."""
    return await _get_dataplane_config(db)
...
async def get_model_uuids(
    ...
    for yaml_file in sorted(models_path.glob("*.yaml")):
        ...
        try:
            ...
            result[name] = raw.get("dataplane_uuid") or None
        except Exception as exc:
            log.warning("Could not read %s: %s", yaml_file.name, exc)
    return result


_GRAFANA_DASHBOARD_UID = "examlops-dataplane"
...
async def get_grafana_panels(
    ...
    """Return Grafana panel info for the DataPlane Live Metrics section."""
...
@router.get("/status")
async def get_dataplane_status(
    db: AsyncSession = Depends(get_db),
    _role: str = Depends(require_role("viewer")),
) -> dict:
    """Probe the bridge status server and return health + stats."""
    cfg = await _get_dataplane_config(db)
    status_url = cfg.get("dataplane_bridge_status_url") or _DEFAULT_BRIDGE_STATUS_URL
