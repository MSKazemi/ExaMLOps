...
def test_scaffold_preview_is_readonly(monkeypatch):
    ...
    out = pipelines.scaffold_preview.invoke({"name": "DemoAD", "task": "anomaly_detection", "task_type": "classification"})
...
def test_scaffold_create_confirmed(monkeypatch):
    ...
    out = pipelines.scaffold_create.invoke({"name": "DemoAD", "task": "anomaly_detection", "task_type": "classification"})
