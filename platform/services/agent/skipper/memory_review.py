    ...
    *,
    ...
    with _conn(db_path) as conn:
        cur = conn.execute(
            ...
            "(task_class, steps_json, success_conditions, operator) VALUES (?,?,?,?)",
            (task_class, json.dumps(steps), success_conditions, operator or config.AGENT_ACTOR),
        )
        return int(cur.lastrowid or 0)


def list_pending(db_path: str | None = None) -> list[dict[str, Any]]:
    """All pending reviews, oldest first."""
    with _conn(db_path) as conn:
        rows = conn.execute(
            "SELECT * FROM procedure_review_queue WHERE status='pending' ORDER BY id"
        ).fetchall()
    return [_row_to_dict(r) for r in rows]


def get(review_id: int, db_path: str | None = None) -> dict[str, Any] | None:
    with _conn(db_path) as conn:
        row = conn.execute(
            "SELECT * FROM procedure_review_queue WHERE id=?", (review_id,)
        ).fetchone()
...
def approve(
    ...
    *,
    ...
    try:
        row = conn.execute(
            "SELECT * FROM procedure_review_queue WHERE id=? AND status='pending'", (review_id,)
        ).fetchone()
    finally:
        conn.close()
    if row is None:
    ...
    *,
    ...
    with _conn(db_path) as conn:
        cur = conn.execute(
            ...
            (reviewer or config.AGENT_ACTOR, reason, review_id),
        )
        n = cur.rowcount
