#!/usr/bin/env bash
# One-time setup for ExaMLOps deployment on lxp-cpu01.
# Run as the deploy user that GitLab CI will SSH as.
# Usage: bash platform/ci/setup_lxp.sh <gitlab-ssh-url> [deploy-path]
set -euo pipefail

REPO_URL="${1:?Usage: $0 <gitlab-ssh-url> [deploy-path]}"
DEPLOY_PATH="${2:-/nfs/share01/examlops}"
...
echo "==> Creating seanerbus-net Docker network (if absent)..."
docker network inspect seanerbus-net > /dev/null 2>&1 \
    || docker network create seanerbus-net

if [ ! -f "$ENV_FILE" ]; then
    echo "==> Scaffolding .env at $ENV_FILE"
    cat > "$ENV_FILE" <<'ENVEOF'
# ExaMLOps — lxp-cpu01 production environment
