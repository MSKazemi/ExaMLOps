...
(``pipelines.usecase`` / ``examlops.usecase``), never by importing ``modelzoo`` in core.

String *specs* (e.g. the loader's legacy-fallback ``"modelzoo.…:X"``) are allowed — only
...
_BANNED_IMPORT = re.compile(r"^\s*(?:from|import)\s+modelzoo\b")
...
def main() -> int:
    ...
    if violations:
        ...
        print(
            "  the platform core / pipeline engine must not import 'modelzoo' directly —\n"
        ...
        return 1
    print("✓ platform ⟂ use-case boundary clean (no direct modelzoo imports in core).")
    return 0
