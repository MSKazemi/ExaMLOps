"""Unit tests for DataPlane bridge Prometheus metrics."""
...
# ── minimal stubs so bridge imports succeed without dataplane installed ─────
...
msgs_stub = types.ModuleType("dataplane_msgs")
...
sys.modules.setdefault("dataplane_msgs", msgs_stub)
...
sb_client_stub = types.ModuleType("dataplane_client")
sb_client_stub.Connection = object
sys.modules.setdefault("dataplane_client", sb_client_stub)
...
def test_all_five_metrics_registered():
    ...
    names = _metric_names()
    assert "dataplane_bridge_up" in names
    assert "dataplane_inferences" in names  # registered as dataplane_inferences_total
    assert "dataplane_inference_errors" in names  # registered as dataplane_inference_errors_total
    assert "dataplane_inference_latency_seconds" in names
    assert "dataplane_retrain_triggers" in names  # registered as dataplane_retrain_triggers_total
...
def test_metrics_output_contains_expected_lines():
    ...
    assert "dataplane_bridge_up" in output
    assert "dataplane_inferences_total" in output
    assert "dataplane_inference_latency_seconds_bucket" in output


def test_inferences_counter_increments():
    before = REGISTRY.get_sample_value("dataplane_inferences_total", {"model": "TEST"}) or 0.0
    bridge._INFERENCES.labels(model="TEST").inc()
    after = REGISTRY.get_sample_value("dataplane_inferences_total", {"model": "TEST"}) or 0.0
    assert after == before + 1


def test_errors_counter_increments():
    before = REGISTRY.get_sample_value("dataplane_inference_errors_total", {"model": "TEST"}) or 0.0
    bridge._ERRORS.labels(model="TEST").inc()
    after = REGISTRY.get_sample_value("dataplane_inference_errors_total", {"model": "TEST"}) or 0.0
    assert after == before + 1
