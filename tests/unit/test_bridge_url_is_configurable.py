...
`SEANERBUS_BRIDGE_STATUS_URL` is the documented way to say where the SeanerBUS bridge is, and the
dashboard has honoured it since the day the container's own loopback made it report "Bridge
offline". The two `exa` commands that probe the same bridge did not: `exa seanerbus status` read a
`seanerbus_bridge_url` attribute off `Config` that no `Config` ever had — so the `getattr` fallback
...
def _hermetic_config(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SEANERBUS_BRIDGE_STATUS_URL", raising=False)


def test_the_default_is_the_host_port_the_stack_publishes():
    from examlops.cli._config import load_config

    assert load_config().seanerbus_bridge_url == "http://localhost:18003"
...
def test_the_documented_env_var_steers_it():
    ...
    os.environ["SEANERBUS_BRIDGE_STATUS_URL"] = "http://bridge.example:8003"
    try:
        assert load_config().seanerbus_bridge_url == "http://bridge.example:8003"
    finally:
        del os.environ["SEANERBUS_BRIDGE_STATUS_URL"]
...
def test_production_probes_the_configured_bridge_not_a_hard_coded_one(monkeypatch):
    ...
    result = production._check_seanerbus(Config(seanerbus_bridge_url="http://elsewhere:9999/"))

    assert asked == ["http://elsewhere:9999/health", "http://elsewhere:9999/stats"]
    assert result.ok is True


def test_seanerbus_status_reads_the_config_field_that_now_exists():
    """The old code asked `getattr(cfg, ..., default)` for an attribute that was never defined."""
    from examlops.cli._config import Config

    assert hasattr(Config(), "seanerbus_bridge_url"), (
        "exa seanerbus status resolves the bridge through this field; without it the command "
