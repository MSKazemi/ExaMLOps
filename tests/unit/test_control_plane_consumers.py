...
def test_the_scan_finds_the_known_consumers():
    ...
    # (`exa approvals` now calls the generated /v1 client, examlops.control_plane_api, which is
    # derived from the contract and guarded by test_control_plane_client_generated.py.)
    for expected in (
    ...
    # the generated client (plan P1.6c), which this extractor deliberately does not read.
    assert len(_call_sites()) >= 35
