...
Large-scale datasets (e.g. the JSC juphoria Day-0 store) may live on a
...
def test_dedicated_dataset_store_env(monkeypatch):
    monkeypatch.setenv("EXAMLOPS_DATA_S3_ENDPOINT", "https://juphoria-s3.fz-juelich.de")
    monkeypatch.setenv("EXAMLOPS_DATA_S3_ACCESS_KEY", "AK")
    monkeypatch.setenv("EXAMLOPS_DATA_S3_SECRET_KEY", "SK")
    assert pg._dataset_store_kwargs("minio") == {
        "endpoint_url": "https://juphoria-s3.fz-juelich.de",
        "access_key": "AK",
        "secret_key": "SK",
    }
...
def test_minio_backend_receives_dedicated_endpoint(monkeypatch):
    ...
    assert backend.endpoint_url == "https://juphoria-s3.fz-juelich.de"
