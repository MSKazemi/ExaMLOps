...
# capnp — needed only at import time of dataplane_msgs; keep setdefault so
...
# dataplane C extension
if "dataplane" not in sys.modules:
    sb_client = types.ModuleType("dataplane")
    sb_client.client = types.ModuleType("dataplane.client")
    sb_client.client.Connection = MagicMock()
    sys.modules["dataplane"] = sb_client
    sys.modules["dataplane.client"] = sb_client.client
...
sys.modules["dataplane_msgs"] = msgs_stub
...
async def test_call_pipeline_logs_res_line(mock_http_client, caplog):
    ...
    job = _make_hpc_job()
    with patch("dataplane_bridge.httpx.AsyncClient", return_value=mock_http_client):
        with caplog.at_level(logging.INFO, logger="dataplane_bridge"):
