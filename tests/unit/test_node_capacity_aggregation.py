...
def test_sql_aggregation_matches_python(db):
    ...
    db.record_node_snapshot("remote", "flux", nodes)

    agg = db.aggregate_node_capacity("remote")["remote"]
...
def test_ttl_cache_serves_stale_within_window_then_invalidates_on_snapshot(db):
    ...
    db.record_node_snapshot("remote", "flux", _nodes([("idle", 4, 32)]))
    first = cap.capacity_summary("remote", ttl=100, _now=1000.0)
    assert first["remote"]["total_gpus"] == 4

    # Directly mutate the table WITHOUT going through record_node_snapshot → cache still serves old.
    with db.get_db() as conn:
        conn.execute("UPDATE hpc_nodes SET gpus = 8 WHERE cluster='remote'")
    cached = cap.capacity_summary("remote", ttl=100, _now=1050.0)  # within TTL
    assert cached["remote"]["total_gpus"] == 4  # served from cache

    # A real snapshot invalidates the cache → fresh value.
    db.record_node_snapshot("remote", "flux", _nodes([("idle", 8, 32)]))
    fresh = cap.capacity_summary("remote", ttl=100, _now=1051.0)
    assert fresh["remote"]["total_gpus"] == 8
...
def test_ttl_zero_disables_cache(db):
    ...
    db.record_node_snapshot("remote", "flux", _nodes([("idle", 4, 32)]))
    cap.capacity_summary("remote", ttl=0)
    with db.get_db() as conn:
        conn.execute("UPDATE hpc_nodes SET gpus = 9 WHERE cluster='remote'")
    # ttl=0 → no cache → reflects the live table immediately.
    assert cap.capacity_summary("remote", ttl=0)["remote"]["total_gpus"] == 9
