...
| `exa pipeline run` | Runs training pipeline(s) locally via Prefect (**mutation** — trains and logs to MLflow). | Train one or all models, optionally pinned to a dataset revision, backend, or HPC cluster. | `exa pipeline run --model JPCP --dataset PM100Dataset --dummy`<br>`exa pipeline run --model JPCP --dataset PM100Dataset --backend minio --project minio-demo`<br>`exa pipeline run --ir jpcp.ir.json --dummy` (train a compiled pipeline-as-code IR, inline scheduler only) |
...
| `exa pipeline show` | Prints a pack model's pipeline IR by name: its per-model registry YAML, or with `--ir` the compiled JSON graph and its content hash (read-only; takes a name, not a path). | Inspect exactly what a model's pipeline is before running or reviewing it. | `exa pipeline show JPCP --ir` |
...
| `exa pipeline distributed suspend backends` | Every suspend backend (ADR 0109) with its honest capability, tiers, preemption verdict and restore timing split. | See what suspend can really preserve before relying on it. | `exa pipeline distributed suspend backends` |
| `exa pipeline distributed suspend capability` | One backend's capability, measured from recorded restores, and why it may not promise preemption. | Check a backend's ceiling. | `exa pipeline distributed suspend capability tiered-training-checkpoint` |
| `exa pipeline distributed suspend snapshot` | Suspends a training run, agent session or vLLM replica and records it (**mutation** — pins state, audited; exit 1 on refusal). | Release compute without losing the run's state. | `exa pipeline distributed suspend snapshot dist-run-7 --backend training-checkpoint` |
| `exa pipeline distributed suspend resume` | Restores a snapshot and records `state_transfer_s` / `communicator_rebuild_s` (**mutation**; exit 1 if it cannot). | Bring a suspended workload back. | `exa pipeline distributed suspend resume <snapshot-id>` |
| `exa pipeline distributed suspend discard` | Releases a snapshot's pin and record; never deletes the checkpoint (**mutation**). | Let retention reclaim a step. | `exa pipeline distributed suspend discard <snapshot-id>` |
| `exa pipeline distributed suspend show` | Shows one suspend record. | Inspect a snapshot's pointer and timing. | `exa pipeline distributed suspend show <snapshot-id>` |
| `exa pipeline distributed suspend list` | Lists suspend records newest first (tenant filter applied before the limit). | Review what is suspended. | `exa pipeline distributed suspend list --status suspended` |
...
| `exa reproduce run` | Rebuild plan + metric-match within tolerance; with `--execute` really rebuilds in five checked steps: detached worktree at the bundle's commit (refuses a bundle built from a dirty tree unless `--allow-dirty-code`), dataset check (dataplane snapshots are verified against their manifest), lockfile-hash + package-by-package check, re-training in the checkout (the real pipeline flow when no `--train-cmd`), metric comparison (`--rtol`); exit 1 on any failure. | Confirm a model can be reproduced from its bundle. | `exa reproduce run JPCP 17 --execute --dummy --rtol 0.05` |
| `exa reproduce verify` | Checks that referenced inputs still exist and hashes match, including dataplane snapshot checksums and the installed-package set (`--allow-env-drift` downgrades package drift to a warning); exit 1 if rotted (R5/GWT-4). | Detect bit-rot / drift of a bundle's inputs (CI gate). | `exa reproduce verify JPCP 17` |
| `exa reproduce show` | Prints the latest signed manifest of a bundle (read-only; `--json` includes the full package set). | Inspect exactly what a bundle captured. | `exa reproduce show JPCP 17` |
...
| `exa feature sync` | **[mutation]** Applies the use-case pack's `features/*.yaml` view definitions to the registry (idempotent, audited); refuses a malformed set. `--dry-run` previews. | Make the registry match the one train/serve definition the pack declares. | `exa feature sync --dry-run` |
| `exa feature status` | Every view's age, TTL, schedule interval, stale/due flags, plus the online-store tier and the serving view in force. | See at a glance which views are stale or due, and what serving uses. | `exa feature status` |
| `exa feature materialize-due` | **[mutation]** Materializes every view whose `materialize_interval_seconds` has elapsed (the control plane runs this on a schedule); `--dry-run` lists them. | Run the schedule by hand, from cron, or to clear a `FeatureViewStale` alert. | `exa feature materialize-due --view fdata_job_features` |
...
| `exa models attest <model> <version> [--path <artifact>] [--sign] [--replace]` | **Mutation.** Records signed SLSA v1 build provenance (in-toto statement in a DSSE envelope, or a Sigstore bundle) and an AI-BOM bound to the artifact digest; refuses bytes that differ from the signed digest. `--dataset`, `--dataset-revision`, `--framework`, `--run-id` enrich it. | Back-fill release evidence for versions registered before provenance was recorded. | `exa models attest JPCP 17 --dataset FData --dataset-revision abc123` |
| `exa models provenance <model> <version> [--output <file>]` | Shows and verifies a version's SLSA v1 provenance (exit 1 when it does not verify: `missing`, `unsigned`, `bad-signature`, `untrusted-key`, `subject-mismatch`); `--output` writes the signed envelope. | Hand an auditor the build provenance of a served model. | `exa models provenance JPCP 17` |
| `exa models release-check <model> <version> [--require signature,bom,provenance]` | CI gate: exit 1 unless the version is signed, has an AI-BOM bound to the signed digest and verified provenance. | Block an unsigned or unattested model in CI before promotion. | `exa models release-check JPCP 17` |
...
| `exa models quantize-gate <model> <version>` | **Mutation (records a gate report).** Mandatory quality-retention gate for a quantized version: its C3 suite scores vs its **base** version's; no gate or missing scores refuses; exits 1 on refusal (ADR 0016 d3). | CI step before promoting a quantized variant. | `exa models quantize-gate JPCP 17-awq` |
...
| `exa embedding migrate` | **Mutation (additive).** Publishes every locally registered encoder to the MLflow encoder registry (ADR 0043 clause 1); one already there is skipped, being the same content-addressed record. `--dry-run` lists what would be published. | Move the registry of record to MLflow before switching `EXAMLOPS_ENCODER_REGISTRY=mlflow`. | `exa embedding migrate --dry-run` |
...
| `exa serve kuberay-manifest` | Renders the Ray multi-model server as a KubeRay `RayService` (every model in the registry behind one Ray cluster), checked against the pinned KubeRay schema; nothing is applied. | Run the dense multi-model Ray path on Kubernetes instead of Compose. | `exa serve kuberay-manifest --image ghcr.io/mskazemi/examlops-ray-serving:v0.61.0 --out ray.yaml` |
| `exa serve verifier-manifest` | Renders the `ClusterStorageContainer` that downloads each model artifact and checks its signature in the pod before the model server starts. | Install verify-before-load on a KServe cluster once. | `exa serve verifier-manifest --image IMAGE \| kubectl apply -f -` |
...
| `exa serve autoscale run` | Runs the autoscale controller: Prometheus signals -> `decide_scale` -> apply, every decision audited. Dry run by default; `--apply` needs `EXAMLOPS_AUTOSCALE_ENABLED=1`, takes a lease, honours a per-cycle storm cap; `--once`, `--applier record\|desired\|ray` (`desired` writes desired replicas for an external owner; `ray` is not built and refuses), `--interval`. | Make scaling decisions execute instead of only being computed on request. **mutation with `--apply`** | `exa serve autoscale run --once` |
| `exa serve autoscale manifest MODEL` | Renders a KEDA `ScaledObject` (`--kind keda`) or a Knative/KServe autoscaling overlay (`--kind knative`) from the model's policy (`--target`, `--namespace`, `--prometheus-url`, `--out`). Read-only; refuses `queue_depth`/`gpu_util` policies (no per-model series). | Hand the autoscaling policy to a cluster-side actuator. | `exa serve autoscale manifest JPCP --kind keda` |
| `exa serve autoscale prefetch` | Plans which models to keep warm or pre-pull from policies plus recent traffic (`--top`). Read-only; the weight cache and activator are not built. | See which models a warm pool should hold. | `exa serve autoscale prefetch` |
| `exa serve autoscale activate MODEL` | Wakes a scaled-to-zero model (`--applier desired\|k8s\|record`) and waits until it is ready (`--timeout`); the cold start is measured and recorded as an audited scale event. | Pre-warm a model before traffic, or recover one left at zero. **mutation** | `exa serve autoscale activate JPCP --applier k8s` |
...
| Command | What it does | Use case | Example |
|---|---|---|---|
| `exa gateway schema test SCHEMA_FILE OBJECT_FILE` | Validates (and optionally `--repair`s) an object against a JSON Schema file or a registered schema name. | Verify/repair model output against a contract. | `exa gateway schema test schema.json out.json --repair` |
...
| `exa rag eval KB` | Scores a KB's retrieval (`context_precision`/`context_recall`, Ragas-backed when installed) through the C2 harness and records it as model `rag:<kb>`; `--items` (JSONL of `{question, relevant_ids}`), `-k`, `--retrieval`, `--backend auto\|ragas\|native`, `--version`, `--alias`, `--no-record`. | Gate a knowledge base on retrieval quality with `exa eval gate`. **mutation** | `exa rag eval kb --items qa.jsonl --alias Production` |
...
| `exa prompt canary` | **(Mutation)** Splits a label across multiple versions by weight (BL-109), so `name@label` resolves to different versions in proportion — audited, C3-gated per candidate, `platform_db` backend only. `--clear` reverts to the single-version pointer. | Stage a prompt change to a fraction of traffic before a full `exa prompt label` promotion. | `exa prompt canary greeting prod --split 2:0.9,3:0.1` |
...
| `exa guardrails checks` | Lists the checks a guardrail policy can compose (built-in, `llm_guard`, plugins) and whether each can run here. | See whether the LLM-Guard extra is installed before naming it in a policy. | `exa guardrails checks` |
| `exa guardrails policy validate` | Validates a declarative guardrail policy file; exits 1 on any error. `--file` (default: the configured policy). | CI gate before rolling out a policy change. | `exa guardrails policy validate --file guardrails.yaml` |
| `exa guardrails policy show` | Shows the effective policy for `--tenant` and `--route` (layers default → tenant → routes). | Check what the gateway will enforce for one tenant's model. | `exa guardrails policy show --tenant acme --route llama3.1:8b` |
...
| `exa agent version register` | Validate an agent manifest (prompts, models, tools, policy pinned by digest or revision) and register it as an immutable, content-addressed version; identical content returns the existing one. | Make an agent's whole behaviour tuple one versioned, promotable thing instead of a prompt label. | `exa agent version register agent.yaml`<br>`exa --json agent version register agent.json` |
| `exa agent version show` | Show one version's pinned tuple and whether its signature verifies; accepts `<agent>@<alias>`. | See exactly what is running under Production. | `exa agent version show jobdoc@Production` |
| `exa agent version list` | List registered versions, newest first, with the aliases pointing at each. | Find the version id to promote or diff. | `exa agent version list --agent jobdoc` |
| `exa agent version card` | Print the A2A-shaped Agent Card of a registered version (content-addressed; skills from its pinned tools; declares no capability the platform does not implement). | Hand another agent or a registry a description of exactly this agent version. | `exa agent version card jobdoc@Staging`<br>`exa agent version card jobdoc --out card.json` |
| `exa agent version diff` | Name the components that differ between two versions (prompt, model, tool, policy, code). | Review what a candidate changes before promoting it. | `exa agent version diff jobdoc@Staging jobdoc@Production` |
| `exa agent alias set` | Point Staging / Canary / Production at a version; Production needs recorded evaluation evidence (calibrated judges) and a valid signature where signing is configured. | Promote an agent version through the same gate a model takes. | `exa agent alias set jobdoc Production av-sha256:...` |
| `exa agent alias show` | Show where an agent's aliases point, and one alias's move history. | Answer "what changed, and who moved it". | `exa agent alias show jobdoc Production` |
| `exa agent alias rollback` | Move an alias back to the version it held before its latest move (not re-gated; audited). | Recover quickly from a bad promotion. | `exa agent alias rollback jobdoc Production --reason regression` |
| `exa agent alias canary` | Start a share of an agent's NEW sessions on its Canary version; existing sessions stay pinned (audited). | Roll a candidate out by session. | `exa agent alias canary jobdoc 10` |
| `exa agent version compat` | State-compatibility verdict between two versions' checkpoint schemas: compatible, incompatible or inert. | Know whether a promotion needs `--state-strategy pin` or `drain`. | `exa agent version compat jobdoc@Production jobdoc@Staging` |
| `exa agent version evidence` | Export a version's evidence pack (tuple, evals with intervals, judges, grants, promotions, rollbacks, audit), digest-sealed. | Hand an auditor one tamper-evident record of an agent version. | `exa agent version evidence jobdoc@Production --out pack.json` |
| `exa agent version reeval` | List re-evaluations enqueued because a model an agent follows was promoted. | See which agent versions must be re-checked after a model move. | `exa agent version reeval --status pending` |
| `exa agent version reeval-resolve` | Close a pending re-evaluation as passed or failed (audited); passed lifts a blocking agent's model pin. | Record the verdict of a follow-binding re-evaluation. | `exa agent version reeval-resolve 12 --outcome passed` |
| `exa agent runtime snapshot` | Compile the agent snapshot (versions, aliases, canary, grants, quotas, migrations) the agent runtime serves from. | Feed a statically stable agent runtime. | `exa agent runtime snapshot --out agent-snapshot.json` |
| `exa agent runtime serve` | Run the agent runtime: the threads/runs HTTP surface, plus a loop that follows the snapshot (keeping the last-known-good), suspends idle sessions and recovers orphaned runs. Loopback unless `--allow-remote`. | Host registered agent versions for tenants without writing a service per agent. | `exa agent runtime serve --snapshot /state/agent-snapshot.json` |
| `exa agent runtime sandboxes` | Show the sandbox isolation each substrate provider offers on this host, measured. | Check whether this host can run code-executing agents for a gVisor tenant. | `exa agent runtime sandboxes` |
...
| `exa mcp scopes` | Shows the OAuth scope each tool needs on the authenticated HTTP transport (`mcp:tools:read`/`write`/`admin`, or `mcp:tool:<name>`); `--reads` limits it to read tools. | Configure scopes at the identity provider for remote MCP clients | `exa --json mcp scopes` |
...
| `exa drift consume-telemetry` | Long-running consumer of `serving.inference_telemetry` on the NATS backbone: writes drift/input-embedding snapshots published by a bridge running with `EXAMLOPS_TELEMETRY_VIA_EVENTBUS=1` (ADR 0123 d4). Needs `EXAMLOPS_NATS_URL`; terminal/service only. | Run wherever `platform.db` is reachable so the serving-plane bridge no longer needs database connectivity. | `exa drift consume-telemetry --wait 2` |
| `exa drift concept <model>` | Concept-drift test on realized error as delayed labels arrive (C5·R1). `--alias`, `--window` (default 50), `--detector builtin\|river-adwin`. | Detect true accuracy decay once ground truth catches up. | `exa drift concept JPCP --window 100` |
...
| `exa drift run-advanced` | **[mutation]** Sweep every model with the concept, label-free and data-quality detectors and write `drift_events` (ADR 0022). `--once`, `--dry-run`, `--model`, `--interval`, `--window`. Needs `EXAMLOPS_DRIFT_ADVANCED_ENABLED=1` for a real run. | Keep concept and data-quality drift visible (and the auto-retrain trigger fed) without anyone typing the detectors. | `exa drift run-advanced --once --dry-run` |
...
| `exa eval evaluators` | Lists evaluator specs (`abs_error`, `string_similarity`, `rouge_l`, `judge:<rubric>`, `deepeval:<metric>` …) and which engine runs each here (native / rapidfuzz / rouge-score / Ragas / DeepEval / gateway). | Pick `--evaluator` specs for `eval run` or `eval online enable`. | `exa eval evaluators` |
| `exa eval online enable <model>` | **[mutation]** Schedule online evaluation of sampled live traffic (ADR 0007). Required `--suite`, `--evaluator/-e` (repeatable); `--source predictions\|tempo`, `--alias`, `--sample`, `--window`, `--judge-model`, `--tenant`. Every spec is validated before saving; audited. | Measure production quality continuously, not only when someone runs a suite. | `exa eval online enable JPCP --suite live -e abs_error -e numeric_match:0.5` |
| `exa eval online disable <model>` | **[mutation]** Stop scheduling a model's online eval (config and results kept); audited. | Pause live scoring during an incident. | `exa eval online disable JPCP` |
| `exa eval online status` | Every online-eval schedule and its latest window's outcome, plus whether the scheduler kill-switch is on. `--tenant`. | See which models are being scored and whether the last window worked. | `exa eval online status` |
| `exa eval online run` | **[mutation]** One cycle (`--once`) or a loop: pull each schedule's window of traffic, sample by `request_hash`, score, persist to the eval store and (with `EXAMLOPS_EVAL_METRICS_TEXTFILE`) rewrite the Prometheus textfile. Needs `EXAMLOPS_EVAL_ONLINE_ENABLED=1`; lease-guarded; `--dry-run` previews. `--model`, `--tenant`, `--interval`. | Run the scheduled online-eval flow (a Compose sidecar or cron). | `EXAMLOPS_EVAL_ONLINE_ENABLED=1 exa eval online run --once` |
...
| `exa slo pair-set <model> <name>` | **[mutation]** Declare a paired `(TTFT, TPOT)` serving SLO (ADR 0117 d2). Required `--ttft-ms`, `--tpot-ms`; `--percentile` (99), `--tight ttft\|tpot`, `--class interactive\|batch\|agent`, `--tenant`. | Say which side of user-perceived latency is tight; one number cannot. | `exa slo pair-set qwen chat --ttft-ms 300 --tpot-ms 40 --tight ttft` |
| `exa slo pair-list` | List declared pair SLOs. `--model`, `--tenant`. | See every latency pair in scope. | `exa slo pair-list --model qwen` |
| `exa slo pair-check <model> <name>` | Evaluate observed samples (`--samples file.json`, `[[ttft_ms, tpot_ms], ...]`) against the pair: both dimensions must hold at the percentile; reports attainment and burn; `--min-samples`. Exit 1 unless `met` (too few samples or an undeclared pair is `no_verdict`, not a pass). | Gate a benchmark run or a canary on paired latency. | `exa slo pair-check qwen chat --samples bench.json` |
| `exa slo spec set <servable>` | **[mutation]** Declare the one SLOSpec of a servable/agent; the objective shape is decided by `--kind` (ADR 0148 d3). `predictive`: `--latency-p99-ms`, `--error-rate`, `--availability`; `generative`: `--pair` (a declared p99 TTFT/TPOT pair), `--goodput-target`; `agentic`: `--task-success` + `--judge`, `--jct-p50-s`, `--jct-p95-s`, `--intervention-rate`, `--cost-per-task-p95-usd`; `--tenant`. An objective of another kind, an out-of-range value or a missing required one is refused. | Give a promotion something to be held to. | `exa slo spec set JPCP --kind predictive --latency-p99-ms 300 --error-rate 0.01` |
| `exa slo spec list` | List declared SLOSpecs. `--servable`, `--kind`, `--tenant`. | See what each servable is held to. | `exa slo spec list --kind agentic` |
| `exa slo spec show <servable>` | One SLOSpec plus its latest recorded verdict. Required `--kind`; `--tenant`. Exit 1 when not declared. | Read the objectives and the last evidence. | `exa slo spec show skipper --kind agentic` |
| `exa slo benchmark record <servable>` | **[mutation]** Store a generative benchmark run from `--file` (`conditions` + `[[ttft_ms, tpot_ms], ...]`); rejected unless it states model, quantization, hardware, engine version, dataset, length distribution, concurrency, a declared SLO pair and whether TTFT includes queue wait (ADR 0143 d5). | Keep TTFT/TPOT/goodput numbers comparable. | `exa slo benchmark record chat-llm --file bench.json` |
| `exa slo benchmark results [servable]` | Stored benchmark results with their conditions, newest first; `--limit`, `--tenant`. | Compare runs across engine versions and hardware. | `exa slo benchmark results chat-llm` |
| `exa slo spec check <servable>` | Evaluate the SLOSpec: `met`, `violated` or `no_verdict` (no data is never a pass). Required `--kind`; `--samples file.json` replaces the platform's own ledgers; `--record` persists the verdict for the promotion gate; `--window-days`, `--min-samples`, `--tenant`. Exit 1 unless `met`. | Gate a run or produce the evidence an armed `slo` gate reads. | `exa slo spec check skipper --kind agentic --samples tasks.json --record` |
...
| `exa hardware profile in-use` | Every profile bound to a running workbench or resolved by a training/serving consumer in the last `--days` (default 7), with the status it got; a deleted version still referenced reports `missing`. Exits 1 on any `degraded`/`unresolvable`/`missing` entry. | See what is sized by a profile, and whether that sizing was ever confirmed. | `exa hardware profile in-use --project research` |
| `exa hardware profile history [name]` | The append-only resolution ledger: which version each consumer resolved, against which cluster, with what status — filterable by `--consumer`/`--project`, newest first, bounded by `--limit`. | Answer "which version of this profile did that run use". | `exa hardware profile history gpu-small --consumer training` |
...
| `exa finops specdecode` | Speculative-decoding acceptance rate + estimated speedup per model/tenant/engine, from windowed token counts (`--model`, `--tenant`, `--days`, `--limit`); speedup is a labelled upper bound (ADR 0016 d4). | See whether a draft model is paying for itself. | `exa finops specdecode --model qwen-7b --days 7` |
| `exa finops task-cost record <task>` | **[mutation]** Record one metered agent-task cost entry: `--component model_call\|tool_call\|sandbox_seconds\|idle_state_gb_hours`, required `--project`; `--cost`, `--quantity`, `--hours`, `--rate`, `--entry-id` (idempotent) (ADR 0148 d4). | Meter what per-token pricing misses. | `exa finops task-cost record T1 --project research --component sandbox_seconds --quantity 120 --rate 0.0001` |
| `exa finops task-cost apportion <pool>` | **[mutation]** Split a hot pool's standby cost across its tasks by a declared `--rule equal\|weighted`; shares sum exactly to `--cost`; audited; one apportioning per `--period`. | Attribute idle hot-pool GPUs to the owning project. | `exa finops task-cost apportion gpu-a --cost 4.8 --project research --task T1 --task T2` |
| `exa finops task-cost show <task>` | One task's ledger: per-component totals, the total (= sum of its entries), and unmetered components. | See what an agent task really cost. | `exa finops task-cost show T1` |
...
| `exa audit checkpoint` | **[mutation]** Sign the current chain head and anchor it to the WORM store (local file or S3 Object-Lock); `--anchor` exits 1 unless durably anchored, `--skip-unchanged` is a no-op for an already-anchored head | Anchor the chain state at a point in time; run from cron as the periodic export | `exa audit checkpoint --anchor --skip-unchanged` |
...
| `exa audit prune` | **[destructive]** Retention: prune events older than `EXAMLOPS_AUDIT_RETENTION_DAYS` without breaking the chain. Dry run by default; `--execute --archive <file>` deletes after a fresh anchored checkpoint, and a signed prune record keeps `exa audit verify` passing (ADR 0028) | Enforce a retention policy on a long-lived audit log | `exa audit prune --execute --archive audit-2026-h1.json` |
...
| `exa audit maintain` | **[destructive]** Scheduled audit maintenance (ADR 0028): sign + WORM-anchor + transparency-log the chain head, and prune under retention when `EXAMLOPS_AUDIT_PRUNE_SCHEDULED=1`; one cycle per cluster lease. Loops by default; `--once` for cron, `--dry-run` to preview | Run the periodic audit export where no control plane runs it | `exa audit maintain --once` |
| `exa audit maintenance-runs` | Recent scheduled audit-maintenance cycles and which step failed | Check the audit schedule is actually running | `exa audit maintenance-runs -n 10` |
| `exa audit verify-transparency` | Re-check checkpoint receipts against the Rekor / Sigstore transparency log (digest, signature, index, Signed Entry Timestamp); exit 1 on a mismatch | Prove checkpoints were published to a log nobody controls | `exa audit verify-transparency` |
| `exa plan list` | List stored agent plans (ADR 0147 d2), newest first, optionally by state | See what agents have proposed and what is still waiting | `exa plan list --state planned` |
| `exa plan show` | Show one plan: intended change, blast radius, approvals, preconditions, outcome | Review a plan before a human approves it | `exa plan show <plan-hash>` |
| `exa ops list` | List operations of your tenant (the control plane's command records) newest first, with states in the MCP Tasks vocabulary (`working`/`input_required`/`completed`/`failed`/`cancelled`); `--state`, `--kind`, `--limit/-n` | See which retrains and dispatches are in flight, done or failed | `exa ops list --state working` |
| `exa ops status` | Show one operation by its handle (the `operation_id` / `command_id` a starting call returned): state, raw control-plane state, whether it is cancellable, flow run and last error | Follow a long-running job without blocking on it | `exa -o json ops status v1:retrain:4f0c1e2a…` |
| `exa ops wait` | Poll one operation until it finishes or `--timeout` seconds pass (default `EXAMLOPS_OPS_WAIT_TIMEOUT`); never blocks forever. Exit 0 completed, 1 failed/cancelled, 124 timed out (the job keeps running); `--timeout 0` looks once | Gate a script or an agent step on a retrain finishing | `exa ops wait v1:retrain:4f0c1e2a… --timeout 600` |
| `exa ops cancel` | **[mutation]** Cancel an operation that is still queued or awaiting retry (audited as `operation_cancel_requested`); a dispatched or finished one is refused with `not_cancellable` and reported honestly | Withdraw queued work before it consumes cluster time | `exa --yes ops cancel v1:retrain:4f0c1e2a…` |
...
| `exa secrets backends` | Show every backend (vault · sops · local · env): configured, reachable/sealed, which one `set`/`rotate` write to — never a value | Check the secrets manager is up before a deploy | `exa secrets backends` |
| `exa secrets refs` | List credential variables as `secret://` reference · file · bootstrap · plaintext and whether each resolves; `--env-file`, `--strict` exits 1 (ADR 0011 cl.2) | Gate a deploy on "no plaintext credential in the env" | `exa secrets refs --env-file .env --strict` |
| `exa secrets lease issue <engine-path>` | Mint a dynamic short-lived credential from an OpenBao engine; prints the lease, fields redacted unless `--reveal` | Give a job a credential that expires | `exa secrets lease issue database/creds/readonly` |
| `exa secrets lease renew <lease-id>` | Extend a lease (`--increment`, engine-capped; audited) | Keep a long job's credential alive | `exa secrets lease renew <lease-id> --increment 3600` |
| `exa secrets lease revoke <lease-id>` | **[mutation]** Revoke a lease now (audited) | End a credential when the job finishes | `exa secrets lease revoke <lease-id>` |
...
| `exa governance features` | Per-feature declarations: the controls each feature serves and the evidence it emits (ADR 0027 decision 2) | See which feature owns a gap | `exa governance features` |
...
| `exa project scope-audit` | Read-only report of `platform.db` tables with no project/tenant scope (scoped / model-scoped / exempt with reason / UNSCOPED); exit 1 on an unscoped table or stale exemption (ADR 0014) | Prove cross-tenant data is not introduced by a new table | `exa project scope-audit` |
| `exa project openfga-sync` | Backfill OpenFGA from the native grants table (dry run unless `--execute`); needs `EXAMLOPS_OPENFGA_URL` + `_STORE_ID` | Move relationship storage to an OpenFGA service | `exa project openfga-sync --execute` |
...
| `exa admission reconcile` | Reclaims leaked quota (ADR 0116): marks reservations whose TTL lapsed `expired`, and asks the scheduler about every committed job reservation (`hpc:<scheduler>:<job id>`), releasing those whose job has ended. A job the scheduler cannot answer for keeps its quota. `--dry-run` changes nothing. **(mutation)** | Return the GPUs of a server that hit its wall time, or of a job whose end was never recorded | `exa admission reconcile --dry-run` |
| `exa admission topology` | Shows the typed resource graph admission reasons over (ADR 0116): vertex counts, each declared scale-up domain's free GPUs and the largest one, built from the `hpc_nodes` inventory plus the site's topology file (`EXAMLOPS_RESOURCE_TOPOLOGY`, else `<config dir>/topology.yaml`). No file = topology unknown, so `scale_up_domain: required` is never promised. Read-only. | Check why a `required`-domain job queues | `exa admission topology --json` |
| `exa admission translate` | Shows how a `JobRequest` JSON file maps onto an execution backend (`--backend mock\|slurm\|flux`): the native resources the adapter receives and the fields that backend cannot enforce. Read-only; nothing is submitted. | See what Flux will not enforce for a request before submitting it | `exa admission translate --request job.json --backend flux` |
