...
(<OLLAMA_HOST> server, port 11436), start the tunnel first. Both vars are set in `.env` and sourced automatically.

| Variable | Default | Purpose |
|---|---|---|
| `AGENT_MODEL` | `llama3.1:8b` | Ollama model name. Via ollama-tunnel: any model from the <OLLAMA_HOST>/<OLLAMA_HOST> list. Must support tool calling. |
