...
| `EXAMLOPS_GATEWAY_QUOTA_REFRESH_SECONDS` | `5` | How often the serving gateway re-checks the serving snapshot for per-tenant quotas (`exa gateway quota set`). A tenant with a snapshot quota uses it instead of `EXAMLOPS_GATEWAY_TENANT_RPM`; if the datastore is unreachable the last quotas read stay in force. |
...
| `EXAMLOPS_PLAN_TTL` | `900` | Seconds a stored agent plan (`plan_change`) stays applicable before `apply_plan` refuses it as expired (ADR 0147 d2). |
...
| `EXAMLOPS_DRIFT_EMBEDDING_SAMPLE_RATE` | `0` (off) | **Drift embedding samples (ADR 0020 clause 4).** Fraction (0-1) of inferences whose raw input-embedding vector the Dataplane bus bridge also writes to the vector store (`drift.embeddings.<model>` collection), so `exa drift input status MODEL --similarity` can measure nearest-neighbour drift against the baseline snapshot. Off by default: a raw input embedding can encode what the input said, so keeping them is an operator decision. The keep/skip choice is a deterministic hash of `(model, job_id)`. Unparseable or non-positive values mean off; above 1 means 1. Writes happen on the telemetry worker thread (or the `serving.inference_telemetry` consumer under `EXAMLOPS_TELEMETRY_VIA_EVENTBUS`), never on the reply path; a failed write is counted (`dataplane_bus_embedding_sample_failures_total`), never raised. |
| `EXAMLOPS_DRIFT_EMBEDDING_CAP` | `500` | Hard per-model ring-buffer size for the drift embedding samples above (clamped to 1-100000). After each write the oldest vectors beyond the cap are evicted and counted (`dataplane_bus_embedding_samples_evicted_total`). |
...
| `EXAMLOPS_REASONING_BUDGET_MODE` | `enforce` | **ADR 0035 cl. 2** over-budget policy, acting only on a request that has a reasoning budget. `enforce` withholds a response that spent more thinking tokens than allowed; `strict` also refuses one whose backend reported no reasoning usage; `flag` serves it and records the breach; `off` disables the gate. Unknown value falls back to `enforce`. |
...
| `LLM_GATEWAY_SEMANTIC_CACHE` | unset (off) | Truthy (`1`/`true`/`yes`/`on`) enables the B3 semantic cache on the LLM gateway service's non-streaming `/v1/chat/completions` path (ADR 0018; a streaming request never uses it). Off by default — caching changes response behaviour for repeat-ish prompts, so it is opt-in per deployment, unlike `EXAMLOPS_GUARDRAIL_MODE` above which always runs. A response tells the caller `x-examlops-cache: hit`/`miss` whenever this is enabled; a request can still skip the cache with `examlops.no_cache: true`. |
...
| `LLM_GATEWAY_ACTIVE_PROBE_INTERVAL_S` | `30` | Cadence (seconds) of the active health-probe loop (`gateway/health.py`, PLAN.md P1): independent of request traffic, every configured provider is probed and the result feeds the same circuit breakers real requests share, so a dead upstream is caught before the next real request reaches it. `0` (or a non-positive value) disables it, same "0 = off" convention `RAY_RELOAD_POLL_SECONDS` uses. State reported at `GET /admin/health`'s `active_probe` block (`running`/`interval_s`/`last_results`). |
| `LLM_GATEWAY_WARM_INTERVAL_S` | `300` | Cadence (seconds) of the warm-preload loop (`gateway/health.py`'s `WarmKeeper`, design spec §5 "Cold start"): a 1-token keep-alive chat to every `gateway.yaml` deployment flagged `warm: true`, so an operator's chosen model never goes cold from being merely quiet. `0` disables it, same convention as the active-probe interval above. State reported at `GET /admin/health`'s `warm` block (`running`/`interval_s`/`last_results`, keyed by deployment). |
...
| `EXAMLOPS_LLM_GATEWAY_URL` | unset | Same base URL, two consumers: the `exa gateway status\|providers\|validate` CLI commands (`gateway_cmd.py`, falls back to `AGENT_LLM_GATEWAY_URL` then `http://127.0.0.1:18020`), and `build_default_router()` (`examlops.gateway`) — when set, the in-process `GatewayClient`'s `default` route answers through the deployed service *before* echo (ADR 0151 d4), so RAG's default generator, `exa gateway chat` and any other in-process caller get a real model without a service redeploy. A separate variable from `AGENT_LLM_GATEWAY_URL` because these run outside the agent container. |
| `EXAMLOPS_LLM_GATEWAY_KEY` | unset | Virtual key `gateway_service_backend()` sends as its bearer to the deployed service (`exa gateway key issue`); unset ⇒ no `Authorization` header, so it only works when the service's `LLM_GATEWAY_AUTH=off`. |
| `LLM_GATEWAY_URL` | unset (dashboard: `http://localhost:18020`) | The dashboard backend's own base URL for the llm-gateway service, probed for liveness on `GET /api/health` alongside every other internal service (`/health` only — never `/admin/*`, so no admin token is ever held by or forwarded through the dashboard for this probe). |
| `PUBLIC_LLM_GATEWAY_URL` | `http://localhost:18020` | The browser-facing URL shown for the llm-gateway service on the dashboard's health/status page, host-rewritten like every other `PUBLIC_*_URL`. |
...
| `AGENT_LLM_GATEWAY_READONLY_MODEL` | unset | A distinct (typically smaller/faster) model for the dashboard copilot's read-only, propose-only graph (ADR 0065). Unset ⇒ the copilot uses the same model as the interactive CLI (unchanged behaviour). Measured on n1 2026-09-24: `llama3.2:3b` prefills a ~7k-token prompt in ~75s cold vs `qwen3:8b`'s ~150-175s — the copilot never executes anything, so a faster, slightly less capable model is a reasonable trade there. |
...
| `EXAMLOPS_LLM_CACHE_PROVIDER` | unset (legacy math) | Provider for the `llm_cache` domain (ADR 0083) — semantic-cache hit-rate/cost-saved reporting (`exa gateway cache stats`, the dashboard caching panel). Unset ⇒ `cache_stats()` keeps computing `hit_rate`/`cost_saved` directly from the summed `cache_events` rows, unchanged. Set to `hit-savings` (the built-in default) or a custom `exa.providers.llm_cache` plugin to route the same numbers through the swappable formula instead (BL-104, 2026-09-24). |
| `EXAMLOPS_LLM_ROUTING_PROVIDER` | unset (`least-cost`) | Provider for the `llm_routing` domain (ADR 0083) — scores a deployment for the gateway's `cost_aware` routing strategy (ADR 0153 d2). Unlike the other two LLM providers this domain always has an active default (`least-cost`: local/site deployments score above external ones; ties keep their existing order), since selecting among deployments is the strategy's entire job — there is no "unconfigured, keep the caller's own math" case. |
| `EXAMLOPS_RAG_QUALITY_PROVIDER` | unset (legacy math) | Provider for the `rag_quality` domain (ADR 0083) — `examlops.rag.context_precision`/`context_recall`. Unset ⇒ the exact existing set-membership fraction, unchanged. Set to `retrieval-lite` (the built-in default, reproduces the same answer to its declared 4-decimal rounding) or a custom plugin (Ragas/MRR/NDCG) to swap the formula. |
