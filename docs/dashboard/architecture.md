...
POST /api/v1/cli/runs {command,args}
   ...
   └─► cli_runner.RUNNER.submit ─► subprocess: python -m examlops.cli --output json --yes <argv>
        ...
        ─► every transition written to `dashboard_cli_runs` (shared store: any replica reads it)
        ─► on finish: audit `cli_run_finished`
...
- **`backend/cli_runner.py`** — isolated subprocess per run; concurrency caps (global + per session,
  ...
  blocks on a full pipe. A cancel that lands before the spawn means the command never starts.
- **`backend/cli_store.py`** — run rows in `dashboard_cli_runs` through `dbconn.connect` (SQLite or
  Postgres): the owning replica writes, every replica reads; cancel is a flag the owner polls;
  history and stored output are bounded; a run whose replica vanished is reported *lost*.
- **`backend/routers/cli.py`** — catalog, runs, cancel, and the admin CLI workspace (upload,
  download, delete; every path through `contain_path`). Every endpoint but cancel is gated by the
  `cliConsole` feature flag (`feature_flags.is_enabled`), so the kill switch is enforced here.
