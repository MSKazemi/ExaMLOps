...
<li data-focus="mlflow,s-promote" data-run="mlflow-promote" data-actor="Training flow" data-line="control"><strong>Lifecycle rules promote it automatically.</strong> The flow walks the model's rules and sets every alias the new version qualifies for — for JPCP, Staging at RMSE ≤ 200, Canary ≤ 100, Production ≤ 50 — and archives the previous Production version. When the model has an eval gate configured, an alias past Staging must also pass it: a failing gate in block mode, a gate that cannot run, or — even in warn mode — one whose judge is uncalibrated stops the version at Staging.</li>
<li data-focus="s-promote,ray" data-run="promote-ray" data-actor="Ray Serve" data-line="control"><strong>Ray Serve reloads the model.</strong> When the Production alias moves, the flow asks Ray Serve to reload at once — if <code>RAY_SERVE_RELOAD_URL</code> or <code>RAY_SERVE_URL</code> is set in the process that serves the flow; otherwise, and for Staging and Canary, the change arrives with the next 60-second alias poll. If MLflow is unreachable, Ray Serve keeps serving the last good version.</li>
<li data-focus="operator,op-gate,mlflow" data-run="operator-gate;gate-mlflow" data-actor="Operator" data-line="human"><strong>Operators can promote with stricter gates.</strong> <code>exa pipeline validate-model JPCP</code> smoke-tests latency and runs the eval gate; <code>exa pipeline promote jpcp --if-rmse-lt 5.0</code> checks the metric condition first (<code>--dry-run</code> stops there), then runs the evaluation, judge-calibration, portability, SLO, compliance, fairness and synthetic-only gates before it moves an alias. <code>--force</code> overrides a failing gate and is recorded.</li>
...
| Admission cap: 4 dispatches in flight globally, 2 per tenant | 409, reported as an identical command already in flight; retry with the same idempotency key |
| The deployment named by `PREFECT_DEPLOYMENT_NAME` exists in Prefect | Prefect's 404 is passed through; `GET /health` shows the configured name as `deployment` |
...
!!! warning "Known issue: control-plane retrains do not dispatch out of the box"
    The control plane dispatches to the deployment named by `PREFECT_DEPLOYMENT_NAME`:
    `examlops_scheduled_training/nightly` in the development stack and
    `training_flow/examlops-dispatch` in the install bundle. `exa pipeline deploy` creates
    neither — it registers `examlops_scheduled_training/examlops-nightly`, a wrapper that
    retrains every model and accepts only `is_dummy` — and no deployment it creates accepts the
    model and dataset a retrain passes. Until a per-model dispatch deployment ships, every
    control-plane dispatch fails: `exa retrain`, the drift trigger, the autopilot, the bus
    bridge, Skipper, MCP, an approval and ModelZoo auto-retrain. `exa pipeline run` and the
    Prefect UI still train directly.

    A request refused at the admission cap also leaves its queued row behind. Until that same
    idempotency key is retried, every later request from the same tenant is refused with 409; a
    request sent without a key gets one that can never be retried, so its row blocks the tenant
    until the control plane's state is cleaned up.
