...
<li data-focus="prom,alert,pager" data-run="prom-alert;alert-pager" data-actor="Alertmanager" data-line="observe"><strong>Rules turn metrics into alerts.</strong> For example Ray Serve errors above 5% for 10 minutes (warning) or 20% for 5 (critical), p99 latency, no models loaded, a bridge down, SLO burn. Every rule links to its runbook section. Alertmanager groups them and pages on critical ones.</li>
...
<li data-focus="writers,audit" data-run="writers-audit" data-actor="Audit trail" data-line="human"><strong>Every governed action is appended to the audit trail.</strong> Retrains, approvals, promotions, cluster admissions and agent writes. Every event's hash covers the one before it, so a changed record breaks the chain.</li>
...
<li data-focus="cp2,outbox,relay,publisher" data-run="cp2-outbox;outbox-relay;relay-publisher" data-actor="Relay" data-line="observe"><strong>Events leave through an outbox.</strong> The control plane writes an event in the same transaction as the change it describes. A relay inside the control plane publishes them every second — to the log by default, or to NATS JetStream — at least once, with stable ids.</li>
</ol>
</div>

!!! note "Drift is watched by the drift trigger, not by an alert rule"
    No alert rule looks at the drift z-score; `DriftEvaluationFailing` only says the evaluator
    itself is failing. The control plane evaluates drift from the bridge's snapshots every minute
    and publishes `drift.status_changed` and `alert.drift` events when a model's status changes.
    `exa drift status` shows it, and `exa drift trigger` or the autopilot act on it — see
...
- [Runbooks](../runbooks/index.md): what each alert means and what to do, and the platform SLOs
