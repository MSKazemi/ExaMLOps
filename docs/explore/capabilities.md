...
description: All 503 exa commands, grouped into the twelve lifecycle areas of exa --help, searchable.
hide:
  - navigation
---
...
Everything ExaMLOps can do is reachable from the `exa` CLI: **503 commands**
...
| Agents & Automation | Ask Skipper, run the autopilot loop and expose tools over MCP. | `ask`, `chat`, `agent`, `agentops`, `autopilot`, `mcp`, `ops` | 42 |
