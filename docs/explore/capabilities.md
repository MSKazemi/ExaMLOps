...
description: All 430 exa commands, grouped into the twelve lifecycle areas of exa --help, searchable.
hide:
  - navigation
---
...
Everything ExaMLOps can do is reachable from the `exa` CLI: **430 commands**
in 66 command groups. They are grouped here the same way `exa --help` groups
...
| Data & Features | Version datasets, validate them and serve features without skew. | `data`, `feature`, `features`, `assets`, `cards` | 30 |
...
| Governance & Security | Approve changes, audit actions, manage secrets and policy. | `auth`, `approvals`, `audit`, `secrets`, `compliance`, `governance`, `policy`, `providers` | 54 |
| Projects & Workspaces | Group models, people, storage and connections into projects. | `project`, `namespace`, `connection`, `workbench` | 35 |
| Platform & Integrations | Operate the stack, back it up and connect external systems. | `stack`, `backup`, `instance`, `upgrade`, `modules`, `events`, `admission`, `exchange`, `seanerbus` | 45 |
