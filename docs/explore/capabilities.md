...
description: All 508 exa commands, grouped into the twelve lifecycle areas of exa --help, searchable.
hide:
  - navigation
---
...
Everything ExaMLOps can do is reachable from the `exa` CLI: **508 commands**
...
| Agents & Automation | Ask Skipper, run the autopilot loop and expose tools over MCP. | `ask`, `chat`, `agent`, `agentops`, `autopilot`, `broker`, `mcp`, `ops` | 47 |
